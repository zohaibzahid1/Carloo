...
const RegistrationForm = () => {
  ...
  const validateForm = () => {
    ...
    const creditCardRegex = /^\d{16}$/; // checks for exactly 16 digits
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      axiosInstance.post('/authentication/register', formData)
        .then(res => {
        ...
        .catch(error => {
          ...
          if (error.response) {
            ...
            if (data.errors && Array.isArray(data.errors)) {
              ...
              } else if (data.message.toLowerCase().includes('server')) {
    ...
    <div className="flex-1 flex flex-col p-12 bg-white overflow-y-auto max-md:p-6 w-full max-w-lg shadow-lg rounded-lg relative z-10 mx-auto">
        ...
        >
            ...
            <button
                ...
                className={`mt-4 h-12 bg-blue-500 text-white border-none rounded-lg font-semibold text-base cursor-pointer transition-all relative overflow-hidden 
                    hover:bg-primary-dark hover:-translate-y-0.5 hover:shadow-lg hover:shadow-primary/15 
                    active:translate-y-0.5 ${isSubmitting ? 'text-transparent' : ''} w-full`}
            >
                {isSubmitting ? (
            ...
            <div className="mt-6 text-center text-gray-600 text-sm w-full">
                ...
                <span
                    ...
                    className="text-primary font-semibold no-underline hover:underline cursor-pointer"
                >
                    Sign in
                </span>
            </div>
